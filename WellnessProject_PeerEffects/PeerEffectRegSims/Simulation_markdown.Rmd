...
#Instrument on Dorm rooms
    ...
    room_SLS_OLS_ratio = room_2SLS_coeff/room_OLS_coeff
    #Store the results
    room_coeff[i] = room_2SLS_coeff
    room_se[i] = room_2SLS_se
    
    dorm_coeff[i] = dorm_2SLS_coeff
    dorm_se[i] = dorm_2SLS_se
    
    floor_coeff[i] = floor_2SLS_coeff
    floor_se[i] = floor_2SLS_se
    OLS_coeff[i] = dorm_OLS_coeff
    
    room_SLS_OLS_ratio[i] = room_SLS_OLS_ratio
  }
  results = data.frame(
  room_coeff = room_coeff,
  room_se = room_se,
  floor_coeff = floor_coeff,
  floor_se = floor_se,
  dorm_coeff = dorm_coeff,
  dorm_se = dorm_se,
  OLS_coeff = OLS_coeff,
  room_SLS_OLS_ratio = room_SLS_OLS_ratio
  )
  return(results)
}
```
...
```{r, echo = FALSE}
ggplot(test) + 
  geom_histogram(aes(x=room_coeff))
```
```{r, echo = FALSE}
ggplot(test) + 
  geom_histogram(aes(x=floor_coeff))
```
```{r, echo = FALSE}
ggplot(test) + 
  geom_histogram(aes(x=dorm_coeff))
```
```{r, echo = FALSE}
ggplot(test) + 
  geom_histogram(aes(x=room_SLS_OLS_ratio))
```
...
We can also show that this bias appears systematically by running the above regression multiple times. If we run the regression 1000 times, we get the following distribution of estimated coefficients.

```{r, include = FALSE}
char_sim = function(num_sims) {
  set.seed(110402024)
  char_coeff = c()
  for (i in 1:num_sims) {
    leave_pop = data.frame(characteristic = rnorm(600, 50, 1))
      ...
      characteristic_model = summary(lm(characteristic_mod ~ leave_out, data = leave_groups))
      char_coeff[i] = characteristic_model$coefficients[2,1]
  }
  results = data.frame(char_coeff = char_coeff)
}
```
```{r, include=FALSE}
char_ex = char_sim(1000)
```
```{r, echo = FALSE}
ggplot(char_ex, aes(x = char_coeff)) + 
  geom_histogram() +
  geom_vline(aes(xintercept = 0))
```

Although there are no peer effects in the data generation process, the above regressions consistently identifies some, as it cannot seperate group specific shocks from peer effects.
...
As can be seen, the failure of the state instrument to satisfy the exclusion restriction results in the 2SLS estimate of the effect of education on wages to be bigger than the OLS estimate, creating the appearance of peer effects. To demonstrate that this bias is systematic, I also ran the above simulation 1000 times and plotted the calculated differences betwee the OLS and 2SLS estimates
```{r, include = FALSE}
ovb_sim = function(num_sims) {
  set.seed(30520244011)
  ols_est
  sls_est
  peer_est
  for(i in 1:num_sims) {
...
To demonstrate that these are both systematic sources of bias, I ran these simulations 1000 times and graphed the results with a histogram.
```{r, include = FALSE}
endog_sim = function(num_sims) {
  set.seed(30520244)
  ols_est = c()
  sls_est = c()
  peer_est = c()
  ...
  for(i in 1:num_sims) {
    ...
    ols_est[i] = return_OLS$coefficients[2,1]
    sls_est[i] = return_2SLS$coefficients[2,1]
    peer_est[i] = return_2SLS$coefficients[2,1] - return_OLS$coefficients[2,1]
    ols_noise_est[i] = return_noise_OLS$coefficients[2,1]
    sls_noise_est[i] = return_noise_2SLS$coefficients[2,1]
    peer_noise_est[i] = return_noise_2SLS$coefficients[2,1] - return_noise_OLS$coefficients[2,1]
  }
  results = data.frame(
    ols_est = ols_est,
    sls_est = sls_est,
    peer_est = peer_est,
    ols_noise_est = ols_noise_est,
    sls_noise_est = sls_noise_est,
    peer_noise_est = peer_noise_est
  )
}
```
```{r, include = FALSE}
endog = endog_sim(1000)
```
First, we'll start by looking at the difference between the OLS and 2SLS estimates in the omitted variable bias case.
```{r, echo = FALSE}
ggplot(endog, aes(x = peer_est)) + 
  geom_histogram(bins = 2000) +
  geom_vline(aes(xintercept = 0)) +
  geom_vline(aes(xintercept = quantile(peer_est,c(0.05, 0.95))[1] , color = "red")) +
  geom_vline(aes(xintercept = quantile(peer_est,c(0.05, 0.95))[2] , color = "red")) +
  coord_cartesian(xlim = c(-75,75))
```
The estimate of peer effects is biased usually slightly above the true value of 0. However, the true value 0 is within the 5th and 95th percentiles of estimated values.
```{r, echo = FALSE}
ggplot(endog, aes(x = peer_noise_est)) + 
  geom_histogram(bins = 2000) +
  geom_vline(aes(xintercept = 0)) +
  geom_vline(aes(xintercept = quantile(peer_noise_est,c(0.05, 0.95))[1] , color = "red")) +
  geom_vline(aes(xintercept = quantile(peer_noise_est,c(0.05, 0.95))[2] , color = "red")) +
  coord_cartesian(xlim = c(-100,100))
```
Still need to come back to this; the 2SLS should make the attentuation from the noise less weak.
